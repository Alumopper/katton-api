...
Katton 把你的 Kotlin 代码组织成**脚本包**。一个脚本包就是一个包含 `manifest.json` 和一些 `.kt` 文件的文件夹（或 `.jar` 文件）。和传统模组不同，脚本包是**不分端的**——同一个包可以同时给服务端和客户端用（Fabric 和 NeoForge），而且全部支持 **`/katton reload` 热重载**。
