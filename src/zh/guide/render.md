...
客户端脚本需要每帧绘制内容时，使用 [HUD 与世界渲染](./render-hud-world.md)。服务端脚本想让客户端看到临时物品模型、但不生成真实实体时，使用 [物品渲染标记](./render-item-markers.md)。需要模糊、染色、灰度或自定义全屏 shader 时，使用 [后处理效果](./render-post-effects.md)。
