...
其实我自己最早也是原版数据包开发者。正是因为我疲于数据包开发里的繁琐流程和各种限制，才做出了 Katton。我希望保留数据包热重载的优点，同时获得接近模组级别的能力。所以如果你也是数据包开发者，不用担心，Katton 的体验会非常熟悉。
...
- `/katton registry` — 查看你的脚本都注册了啥（Paper 上不可用）
- `/katton status` — 检查 Katton 是否正常运行

> [!NOTE]
> 在 Paper 上，命令更为精简，没有 `registry` 和 `debug` 子命令。

完整列表请看 [命令](../quickstart/commands.md) 页面。
