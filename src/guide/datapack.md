...
In datapacks, we will use functions with `#load` tag to execute some code when the datapack is loaded, and use functions with `#tick` tag to execute code every tick. In Katton, these cases are usually better expressed as event listeners. You can use [`ServerEvent.onStartServerTick`](../api/fabric/event/ServerEvent.md#serverevent-onstartservertick) every tick, and [`ServerEvent.onServerStarted`](../api/fabric/event/ServerEvent.md#serverevent-onserverstarted) when the server starts. Check out the [Events](events.md) page for more details.
...
Check the [Commands](../quickstart/commands.md) page for the full list.
