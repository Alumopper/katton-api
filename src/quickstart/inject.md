...
To insert code into an existing method, you can use `insertBefore` or `insertAfter` function. Specially, use `injectConstructorBefore` or `injectConstructorAfter` to inject code into constructors. 
...
// Remember to call the function =w=
@Suppress("unused")
